...
- `getRegions` fail-soft ke `[]` kalau `GET /public/regions` gagal — TANPA fallback ini homepage & `/cari` 404 TOTAL begitu `OMAHE_API_SEARCH=1` dinyalakan (ditemukan lewat verifikasi live, bukan cuma baca kode). **Update 2026-09-23**: endpoint-nya kini ADA (epic `LOKASI-02` repo `perumahan`: kabupaten/kota dari perumahan aktif ber-unit tersedia) — gap §5 tertutup; fallback tetap dipertahankan untuk gangguan backend. Pelajaran: gap yang dicatat "bukan blocker" WAJIB dibuatkan epic di repo `perumahan`, kalau tidak ia tidak pernah masuk antrean kerja (gap ini sempat terbuka 9 hari).

Diverifikasi live 2026-09-15: seed data nyata ke Postgres dev `perumahan` (developer + 2 perumahan salah satunya bertanda migrasi + unit), `OMAHE_API_BASE_URL`/`OMAHE_API_SEARCH=1` di `.env` lokal, `bun run dev` bareng backend — homepage, `/cari`, `/developer`, `/developer/:slug`, `/perumahan/:slug` (termasuk tenant migrasi) semua render data asli, HTTP 200, tanpa `undefined`/`NaN`. Data uji sudah dibersihkan dari dev DB. Detail shape final: `docs/api-contract.md` §Status (kedua gap ditandai selesai; `GET /public/regions` §5 menyusul selesai 2026-09-23 lewat `LOKASI-02`).
...
Murni B2C (bukan rekrutmen developer/mitra — itu tetap `SITE-01` di repo `perumahan`, terpisah): `/` (homepage+search), `/cari` (hasil per-unit), `/developer` (direktori PERUSAHAAN developer), `/developer/:companySlug` (profil developer + daftar proyeknya), `/perumahan/:slug` (detail satu proyek, Omahe-rendered, CTA ke `/ajukan/:slug?ref=...`), `/artikel` (index blog content-marketing) + `/artikel/:slug` (detail, prerendered), `/kpr` (kalkulator), `/tentang`, `/kontak`, `/privasi`, `/syarat-ketentuan` (lima halaman statis terakhir kini di-prerender). Detail per **tipe rumah** kini halaman sendiri `/perumahan/:slug/tipe/:tipeSlug` (UNIT-05, 2026-09-23: galeri khusus tipe + spesifikasi Markdown tenant yang WAJIB dirender lewat `landing/src/lib/markdown.ts`, bukan `marked`); detail per unit fisik (`/unit/:id`) tetap ditunda. Kontak WhatsApp/telepon/email Omahe diatur superadmin di admin `perumahan` (ADMIN-05, `/public/app-settings`, fallback `SITE` di `config.ts`). Rincian: `docs/user-story.md` §Site map.

Domain final: **`www.omahe.co.id`** (diganti dari rencana awal `omahe.id`, 2026-09-20 — apex `omahe.co.id` redirect ke www; DNS sudah mengarah ke Vercel). Setiap card properti wajib ada tombol **WhatsApp + Telepon** (nomor kontaknya belum diputuskan sumbernya — perumahan/developer/marketing, lihat `docs/user-story.md` §Kontak langsung di card). Desain **mobile-first** wajib (pola: hamburger nav, filter jadi chip horizontal, sticky bottom CTA di halaman detail) — lihat `docs/user-story.md` §Mobile-first.
